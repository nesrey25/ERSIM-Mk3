...
private:
    ...
    long int clock; //used to keep track of how long the sim has been running for
    double numTreated; //number of patients successfully treated
    double totalWait; //used with numTreated to find average wait time
    std::vector<std::string> names; //uproducing a random name
    ...
    void updateDoctors(); //update of doctors 
